...
resource "aws_route" "peering-to-ansible-vpc" {
  ...
  destination_cidr_block    = data.aws_vpc.K8s-Vpc-vpc.cidr_block
...
resource "aws_route" "peering-from-ansible-vpc" {
  route_table_id            = data.aws_route_table.K8s-Vpc-rtb-public.id
